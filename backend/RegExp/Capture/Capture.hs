...
modify
  :: (GenMonad m, GenMonadConstraint m ((), s)) => (s -> s) -> StateT s m ()
...
parentheses
  :: (Show a, Show c, Show (m ())) => CaptureMonadicRegExp m c a -> String
...
nullableProper (CaptureCurrent (c, inv_xs) e) = modify (Map.insert c xs)
  >> nullableProper e
  where xs = maybe [] reverse inv_xs
nullableProper (Captured c) = get >>= \ctxt -> case Map.lookup c ctxt of
  Just [] -> return $ return ()
  _       -> return mempty
...
deriveBySymbProper
  :: ( GenMonadConstraint m2 (CaptureMonadicRegExp m1 k a, Map k [a])
     ...
     , Semimodule
         ...
         (m2 (CaptureMonadicRegExp m1 k a, Map k [a]))
...
deriveBySymbProper a (Conc e1 e2) = liftA2StateT
  (<>)
...
deriveBySymbProper at (CaptureCurrent (c, inv_xs) e) =
 ...
 where
  inv_xs' = case inv_xs of
    Nothing -> Just [at]
    Just xs -> Just (at : xs)
deriveBySymbProper at (Captured c) = get >>= \ctxt ->
